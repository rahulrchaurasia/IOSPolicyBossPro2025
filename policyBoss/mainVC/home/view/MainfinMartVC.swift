...
//        guard let data = UserDefaults.standard.data(forKey: Constant.deeplink) else { return }
//        
...
//            
//            // 1. Safely extract product_id (handling both String and Int types)
//            let rawProductId = deepLinkData["product_id"]
...
//    }
    ...
    @objc func NotifyData(notification : Notification){
        ...
        //005 tempary comented for viewModel Call
//        self.userconstantAPI()
//        self.getdynamicappAPI()
        
        Task {
            
            await viewModel.loadInitialData(sendDeviceDetails: false)
        }
...
//            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        if(popUpbackgroundView.isHidden == false)
        {
            // Make sure indexPath.row is in bounds
                   guard indexPath.row < callingDashboardModel.count else {
...
//          cell1.callingDelegate = self
            ...
            let model = callingDashboardModel[indexPath.row]

                    cell1.managerNameLbl.text = model.EmployeeName
                    cell1.mobLbl.text = model.MobileNo
                    cell1.emailLbl.text = model.EmailId
                    cell1.levelLbl.text = model.Designation
                    cell1.callingDelegate = self
            ...
            if(indexPath.section == 0)
            {
                ...
                // check New Product
                if(dynamicDashboardModel[indexPath.row].IsExclusive == "Y" )
                {
...
//        
        let scannerView = ScannerScreen(
            viewModel: viewModel,
            onResult: { [weak self] message in

                self?.handlePostLoginSuccess(
                    strMessage: message
                )
            },
    ...
    private func handlePostLoginSuccess(strMessage : String) {
            // Refresh UI dashboard data here in MainVC since authentication passed
          
        let snackbar = TTGSnackbar.init(message: strMessage, duration: .long)
        snackbar.show()
...
//                    UserDefaults.standard.set(String(describing: androidproattendanceEnable), forKey: getSharPrefernce.attendanceEnable)
                   ...
                   // self.testDeepLink() //Note Fot testing of deeplink
                    ...
                    print("UserConstant Error : \(errorData.errorMessage)")
...
////                                
                                ...
                                let model = DynamicDashboardModel(
                                            ...
                                            dashboard_type: String(aObject["dashboard_type"] as? Int ?? 0),
                                           
                                            ProdId:  String(aObject["ProdId"] as? Int ?? 0),
                                            ProdName: aObject["menuname"] as? String ?? "",
                    ...
                    print("Dynamic  Error : \(errorData.errorMessage)")
                }, onForceUpgrade: {errorData in})
    ...
    func getDeviceDetails(){
        ...
        if Connectivity.isConnectedToInternet()
        {
            ...
            let parameter  :[String: AnyObject] = [
                ...
                "App_Version":   Configuration.appVersion as  AnyObject
            ...
            Alamofire.request(url, method: .post, parameters: parameter,encoding: JSONEncoding.default,headers: FinmartRestClient.headers).responseJSON(completionHandler: { (response) in
                ...
                case .failure(let error):
                    ...
                    print("getDeviceDetails Error : \(error)")
                }
            })
    ...
    func getLoginToken()  {
        ...
        if Connectivity.isConnectedToInternet()
        {
            ...
            let parameter  :[String: AnyObject] = [
                ...
                "app_version":Configuration.appVersion as AnyObject,
                "fbaid" : UserDefaultsManager.shared.getFbaId() as AnyObject
        ...
        alertView.show()
            ...
            Task {
                do {
                    ...
                    let result = try await vmLogin.getUserCallingDetail()
                    
                    DispatchQueue.main.async {  // Update UI on main thread
                        alertView.close()
                        self.view.layoutIfNeeded()
                        
                        if result.1 == "0",let response = result.0 {
                            //  Map MasterData to your model
                            
                            let masterData : [UserCallingData] = response.MasterData
...
//                                let model = CallingDashboardModel(
...
//                            self.callingDashboardModel = masterData.map {
...
//
                            ...
                            for index in 0...(masterData.count) - 1 {
                                
                                let aObject = masterData[index]
                                
                                let model  = CallingDashboardModel(MobileNo: aObject.MobileNo ,
                                     EmailId: aObject.EmailId ,
                                    EmployeeName: aObject.EmployeeName ,
                                    Designation: aObject.Designation
                                )
    ...
    func usercallingAPI2()
    {
        ...
        alertView.show()
            ...
            print("FBAID and SSSID \(FBAId) , \(SSID)")
...
//                                               info: "http://origin-cdnh.policyboss.com/fmweb/GroupHealthCare/update.html"))
    ...
    func callWebViewUsingDeeplink(actionId: String, title: String = "", urlToLoad: String = "") {
        ...
        case  "DB": // Standard or Dashboard Web View
            ...
            let subSSID = UserDefaultsManager.shared.getSubUserSsId() ?? ""
            let appVersion = Configuration.appVersion
            let deviceID = UIDevice.current.identifierForVendor?.uuidString ?? Configuration.deviceID
            
            var finalUrl = urlToLoad
            if sanitizedId == "DB" || !finalUrl.contains("ss_id") {
        ...
        default:
            // Fallback for custom or dynamic deep link URLs passed from outside
            if !urlToLoad.isEmpty {
                ...
                var appendURL = urlURLWithParameters(base: urlToLoad, ssid: SSID, fbaId: FBAId, subFbaId: subFBAID, subSsid: subSSID, ip: ipAddress, version: appVersion, device: deviceID, title: title)
                
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    self.callDeepLinkAndPushNotifyWebView(dynamicUrl: appendURL, dynamicName: title)
                }
            } else {
    ...
    private func urlURLWithParameters(base: String, ssid: String, fbaId: String, subFbaId: String, subSsid: String, ip: String, version: String, device: String, title: String) -> String {
        var appendURL = base
        if !appendURL.contains("ss_id=") {
            appendURL += "&ss_id=\(ssid)"
        }
        if !appendURL.contains("fba_id=") {
            appendURL += "&fba_id=\(fbaId)"
        }
        appendURL += "&sub_fba_id=\(subFbaId)&ip_address=\(ip)&mac_address=\(ip)&app_version=\(version)&device_id=\(device)&login_ssid=&sub_ss_id=\(subSsid)"
        return appendURL
    }
    
    
    // Mark : Calling Push and Deeplink calls
//    func callWebViewUsingDeeplink1(  ProdId : String, ProdTitle : String ,ProdURL : String){
//        
// 
//        dismissAll(animated: false)
//        
...
extension MainfinMartVC {
    ...
    private func bindViewModel() {
        ...
        //🔥 Observe User Dashboard data
        viewModel.$userDashboardModel
            .receive(on: RunLoop.main)
            .sink { [weak self] newData in
                self?.userDashboardModel = newData
            }
            .store(in: &cancellables)
        
        //🔥 Observe  Dynamic Dashboard data
        viewModel.$dynamicDashboardModel
            .receive(on: RunLoop.main)
            .sink { [weak self] newData in
                guard let self = self else { return }
                self.dynamicDashboardModel = newData
               // print("VC received dynamic count: \(newData.count)")
        ...
        viewModel.$allDataLoaded
            ...
            .sink { [weak self] _ in
                ...
                // CRITICAL: Call the final handler that copies data and reloads UI
                
                
                // ✅ Reload only once both arrays are populated
                if !self.userDashboardModel.isEmpty &&
                    !self.dynamicDashboardModel.isEmpty {
                    self.handleDataAfterApICall()
                } else {
                    // wait a fraction if Combine updates are still pending
...
extension MainfinMartVC {
    ...
    func handleDataAfterApICall() {
        
        print("✅ User count: \(userDashboardModel.count)")
        print("✅ Dynamic count: \(dynamicDashboardModel.count)")
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            // 2. App version check
            self.verifyVersion()
            // 3. WebEngage analytics
            self.setupWebEngageAnalytics()
